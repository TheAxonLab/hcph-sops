...
- [ ] Visualize all reportlets in the `Summary` section:
    - [ ] Inspect the shell-wise joint distribution of SNR vs FA and apply the corresponding [exclusion criteria](qaqc-criteria-unprocessed.md#shell-wise-joint-distribution-of-snr-vs-fa-in-every-voxel).
    - [ ] Scroll down to the fractional anisotropy (FA) map and apply the [exclusion criteria](qaqc-criteria-unprocessed.md#fractional-anisotropy-map).
    - [ ] Navigate to the mean diffusivity (MD) map and apply the [exclusion criteria](qaqc-criteria-unprocessed.md#mean-diffusivity-map).
- [ ] Continue with the `DWI shells` section. For each b-value, proceed as follows:
    - [ ] Visualize the first flickering mosaic presenting the average and the standard deviation across volumes in that DWI shell and apply the [exclusion criteria](qaqc-criteria-unprocessed.md#voxel-wise-average-and-standard-deviation-across-volumes-in-a-single-dwi-shell).
    - [ ] Scroll down to the view of the background and apply the [exclusion criteria](qaqc-criteria-unprocessed.md#view-of-the-background-of-the-voxel-wise-average-of-a-single-dwi-shell).
- [ ] Proceed as [earlier](#mriqc-failed) if errors are reported within the "Errors" section.
