...
### Automated intake of a routine session with `add_session.sh`

!!! tip "For a standard session, the whole imaging-conversion chain below is automated by a single script"

    Once the [DICOMs have been downloaded from the PACS](#download-the-data-from-the-pacs-with-pacsman-only-authorized-users), the BIDS conversion, the clean-up, the addition to the *DataLad* dataset, the *PyBIDS* index refresh, the pushes to *GitHub* and the *RIA store* (the step that uploads the data to the HPC), and the archival of the DICOMs can all be run in one step.
    The script must be run from the root of the *DataLad* dataset working tree, and it opens a pull request against the dataset repository (linking the originating `scan` issue) so the new session can be reviewed before it is merged.

- [ ] Run the automated intake script on the downloaded DICOM session folder:

    ``` bash title="One-command session intake"
    bash {{ secrets.data.sops_clone_path | default('<path>') }}/code/datalad/add_session.sh <path-to-DICOM-session-folder>
    ```

    !!! important "Host- and study-specific values are configurable"

        Paths, the subject label, the participant-id prefix used to parse the session code, the reviewer, and similar settings are read from environment variables with sensible defaults — see the configuration block at the top of the script.

??? info "What `add_session.sh` does, step by step"

    ``` {.bash linenums="1"}
{% filter indent(width=4) %}
{% include 'code/datalad/add_session.sh' %}
{% endfilter %}
    ```

The remainder of this section documents the same steps **manually**. Follow the manual procedure for a non-standard session (e.g., partial acquisitions or conversion errors that need inspection), or when setting up for the first time.
...
- [ ] Run the *BIDS Validator* to check the *formal* quality of the dataset (filenames, homogeneity of modalities and parameters across sessions, etc.)
    ...
    docker run -ti --rm -v {{ secrets.celine_paths.sherlock | default('/path/to/data/') }}/hcph/:/data:ro bids/validator /data
    ```
